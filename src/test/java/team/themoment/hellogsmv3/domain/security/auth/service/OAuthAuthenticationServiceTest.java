...
class OAuthAuthenticationServiceTest {
  ...
  void setUp() {
    ...
    oAuthAuthenticationService =
        new OAuthAuthenticationService(oAuthProviderFactory, memberRepository);
    ReflectionTestUtils.setField(
        oAuthAuthenticationService, "sessionTimeout", Duration.ofSeconds(10800));
  }
  ...
  class Describe_execute {
    ...
    class Context_with_valid_provider_and_code_and_existing_member {
      ...
      void it_completes_oauth_authentication_and_sets_security_context() {
        try (MockedStatic<SecurityContextHolder> mockSecurityContextHolder =
            mockStatic(SecurityContextHolder.class)) {
          mockSecurityContextHolder
              .when(SecurityContextHolder::createEmptyContext)
          ...
          assertTrue(
              oAuthToken.getAuthorities().contains(new SimpleGrantedAuthority("OAUTH2_USER")));
          assertTrue(
              oAuthToken.getAuthorities().contains(new SimpleGrantedAuthority("SCOPE_email")));
          assertTrue(
              oAuthToken
                  .getAuthorities()
                  .contains(new SimpleGrantedAuthority(Role.APPLICANT.name())));
    ...
    class Context_with_invalidated_session {
      ...
      void setUp() {
        ...
        given(session.getCreationTime())
            .willThrow(new IllegalStateException("Session invalidated"));
        given(session.getLastAccessedTime())
            .willThrow(new IllegalStateException("Session invalidated"));
