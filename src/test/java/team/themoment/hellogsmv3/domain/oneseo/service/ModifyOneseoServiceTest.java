...
class ModifyOneseoServiceTest {
  ...
  @Mock private ScreeningChangeHistoryRepository screeningChangeHistoryRepository;
  @Mock private EntranceTestResultRepository entranceTestResultRepository;
  @Mock private EntranceTestFactorsDetailRepository entranceTestFactorsDetailRepository;
  @Mock private OneseoService oneseoService;
  @Mock private MemberService memberService;
  @Mock private LambdaScoreCalculatorClient lambdaScoreCalculatorClient;

  @InjectMocks private ModifyOneseoService modifyOneseoService;
  ...
  class Describe_execute {
    ...
    MiddleSchoolAchievementReqDto middleSchoolAchievementReqDto =
        MiddleSchoolAchievementReqDto.builder()
            .achievement1_2(null)
    ...
    class Context_with_valid_member_id_and_request_data {
      ...
      void it_modifies_and_saves_existing_entities() {
        ...
        OneseoPrivacyDetail oneseoPrivacyDetail =
            OneseoPrivacyDetail.builder().id(1L).oneseo(oneseo).graduationType(CANDIDATE).build();

        MiddleSchoolAchievement middleSchoolAchievement =
            MiddleSchoolAchievement.builder().id(1L).oneseo(oneseo).build();
        ...
        ArgumentCaptor<OneseoPrivacyDetail> oneseoPrivacyDetailCaptor =
            ArgumentCaptor.forClass(OneseoPrivacyDetail.class);
        ArgumentCaptor<MiddleSchoolAchievement> middleSchoolAchievementCaptor =
            ArgumentCaptor.forClass(MiddleSchoolAchievement.class);
        ...
        assertEquals(
            oneseoPrivacyDetail.getOneseo().getId(), capturedPrivacyDetail.getOneseo().getId());
        ...
        assertEquals(
            middleSchoolAchievement.getOneseo().getId(), capturedAchievement.getOneseo().getId());
      ...
      void it_change_screening_entity_save() {
        ...
        modifyOneseoService.execute(oneseoReqDto, memberId);
        ArgumentCaptor<WantedScreeningChangeHistory> screeningChangeHistoryArgumentCaptor =
            ArgumentCaptor.forClass(WantedScreeningChangeHistory.class);

        verify(screeningChangeHistoryRepository)
            .save(screeningChangeHistoryArgumentCaptor.capture());
        verify(lambdaScoreCalculatorClient).calculateScore(any(LambdaScoreCalculatorReqDto.class));

        WantedScreeningChangeHistory capturedScreeningChangeHistory =
            screeningChangeHistoryArgumentCaptor.getValue();
    ...
    class Context_with_invalid_member_id {
      ...
      void setUp() {
        doThrow(
                new ExpectedException(
                    "해당 지원자의 원서를 찾을 수 없습니다. member ID: " + memberId, HttpStatus.BAD_REQUEST))
            .when(memberService)
      ...
      void it_throws_an_exception() {
        ExpectedException exception =
            assertThrows(
                ExpectedException.class, () -> modifyOneseoService.execute(oneseoReqDto, memberId));
      ...
      void setUp() {
        ...
        doThrow(
                new ExpectedException(
                    "해당 지원자의 원서를 찾을 수 없습니다. member ID: " + memberId, HttpStatus.BAD_REQUEST))
            .when(oneseoService)
            .findByMemberOrThrow(existingMember);
      }
