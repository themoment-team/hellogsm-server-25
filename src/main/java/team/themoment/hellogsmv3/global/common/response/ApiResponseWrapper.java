...
public class ApiResponseWrapper implements ResponseBodyAdvice<Object> {
  ...
  private static final String[] NOT_WRAPPING_URL = {
    "/api-docs/**", "/hello-management/prometheus/**"
  };
  ...
  public Object beforeBodyWrite(
    ...
    CommonApiResponse<Object> commonApiResponse =
        new CommonApiResponse<>(HttpStatus.OK, HttpStatus.OK.value(), "OK", body);
  ...
  private static CommonApiResponse<Object> exceptionResponse(
      ServerHttpResponse response, Map<String, Object> bodyMap) {
    if (bodyMap.containsKey("status")) {
      int statusCode = (int) bodyMap.get("status");
      if (statusCode >= 400 && statusCode < 600) {
        HttpStatus status = HttpStatus.valueOf(statusCode);
        CommonApiResponse<Object> errorResponse =
            CommonApiResponse.error(status.getReasonPhrase(), status);
