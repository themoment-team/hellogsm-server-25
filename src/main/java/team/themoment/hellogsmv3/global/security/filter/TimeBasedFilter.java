...
public class TimeBasedFilter extends OncePerRequestFilter {
  ...
  public TimeBasedFilter addFilter(
      HttpMethod httpMethod, String uri, LocalDateTime startTime, LocalDateTime endTime) {
  ...
  protected void doFilterInternal(
    ...
    try {
      ...
      response.sendError(
          HttpServletResponse.SC_BAD_REQUEST, "유효하지 않은 HTTP 메서드 입니다. : " + request.getMethod());
      return;
    }
    ...
    if (urlRequestPeriods.containsKey(requestPeriodKey)) {
      ...
      if (requestPeriod.isWithinRange(currentTime)) {
        ...
        String message =
            String.format(
                "%s 요청이 거부되었습니다. " + "현재 시간: %s, 해당 요청은 %s ~ %s 이내에만 처리 가능합니다.",
