...
public class QueryTestResultService {
  ...
  public FoundTestResultResDto execute(
      Long memberId, String code, String phoneNumber, String oneseoCode, TestType testType) {
  ...
  private Oneseo findOneseo(String phoneNumber, String oneseoCode, TestType testType) {
    ...
    return testType.equals(FIRST)
        ? oneseoRepository
            .findByGuardianOrTeacherPhoneNumberAndSubmitCode(phoneNumber, oneseoCode)
            .orElseThrow(
                () ->
                    new ExpectedException(
                        "해당 전화번호, 접수번호로 작성된 원서를 찾을 수 없습니다.", HttpStatus.BAD_REQUEST))
        : oneseoRepository
            .findByGuardianOrTeacherPhoneNumberAndExaminationNumber(phoneNumber, oneseoCode)
            .orElseThrow(
                () ->
                    new ExpectedException(
                        "해당 전화번호, 수험번호로 작성된 원서를 찾을 수 없습니다.", HttpStatus.BAD_REQUEST));
  }
