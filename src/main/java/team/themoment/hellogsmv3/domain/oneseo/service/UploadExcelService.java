...
public class UploadExcelService {
  ...
  public void execute(MultipartFile file) {
    try (Workbook workbook = createWorkbookFromFile(file)) {
      ...
      Map<String, EntranceTestResult> queryResult =
          oneseoRepository.findEntranceTestResultByExaminationNumbersIn(
              examinationNumberToProcess.stream().toList());
  ...
  private Map<String, SecondTestResultDto> getExcelTestResult(Sheet sheet, Workbook wb) {
    ...
    for (int i = 1; i <= sheet.getLastRowNum(); i++) {
      ...
      BigDecimal competencyEvaluationScore =
          readScoreCell(row, CellIndex.COMPETENCY_EVALUATION_SCORE, i, evaluator);
      ...
      excelResults.put(
          examinationNumber, new SecondTestResultDto(competencyEvaluationScore, interviewScore));
    }
    if (!duplicateExaminationNumbers.isEmpty()) {
      throw new ExpectedException(
          "다음 수험번호가 중복되었습니다: " + String.join(", ", duplicateExaminationNumbers),
  ...
  private void validateNotFoundExaminationNumbers(Set<String> expected, Set<String> found) {
    Set<String> notFoundExaminationNumbers =
        expected.stream()
  ...
  private void saveAllEntranceTestResults(
      Map<String, EntranceTestResult> resultToModify,
      Map<String, SecondTestResultDto> excelResults) {
    resultToModify.forEach(
        (examinationNumber, entranceTestResult) -> {
  ...
  private BigDecimal readScoreCell(
      Row row, CellIndex cellIndex, int rowIdx, FormulaEvaluator evaluator) {
    ...
    try {
      ...
      throw new ExpectedException(
          positionMsg(rowIdx, cellIndex, "점수 형식이 올바르지 않습니다."), HttpStatus.BAD_REQUEST);
    }
    score = score.setScale(2, RoundingMode.HALF_UP);
    if (score.compareTo(BigDecimal.ZERO) < 0 || score.compareTo(new BigDecimal("100")) > 0) {
      throw new ExpectedException(
          positionMsg(rowIdx, cellIndex, "점수는 0 이상 100 이하의 값이어야 합니다."), HttpStatus.BAD_REQUEST);
    }
    return score;
  }

  private String readTextCell(
      Row row, CellIndex cellIndex, int rowIdx, FormulaEvaluator evaluator) {
    Cell cell = row.getCell(cellIndex.getIndex());
    if (cell == null || cell.getCellType() == CellType.BLANK) {
      throw new ExpectedException(
          positionMsg(rowIdx, cellIndex, "필수 셀이 비어있습니다."), HttpStatus.BAD_REQUEST);
    }
    String value;
