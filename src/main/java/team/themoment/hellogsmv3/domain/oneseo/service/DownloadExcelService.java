...
public class DownloadExcelService {
  ...
  private static final Map<Screening, String> SCREENING_DISPLAY_MAP =
      Map.of(
          Screening.GENERAL,
          "일반전형",
          Screening.SPECIAL,
  ...
  private static final Map<YesNo, String> PASS_YN_DISPLAY_MAP =
      Map.of(
          YesNo.YES, "합격",
          YesNo.NO, "불합격");
  ...
  private List<List<String>> getCombinedExtraScreeningDataOptimized() {
    ...
    List<Oneseo> extraAdmission =
        oneseoRepository.findAllByScreeningWithAllDetails(Screening.EXTRA_ADMISSION);

    List<Oneseo> combinedList =
        Stream.concat(extraVeterans.stream(), extraAdmission.stream()).collect(Collectors.toList());
  ...
  private List<List<String>> processBatch(List<Oneseo> batchOneseoList, int startIndex) {
    ...
    for (int i = 0; i < batchOneseoList.size(); i++) {
      ...
      List<String> rowData =
          createRowData(oneseo, entranceTestResult, privacyDetail, startIndex + i);
  ...
  private List<String> createRowData(
      Oneseo oneseo,
    ...
    return List.of(
        ...
        safeToString(oneseoPrivacyDetail.getStudentNumber()),
        ...
        getScoreString(
            entranceTestResult.getEntranceTestFactorsDetail().getArtsPhysicalSubjectsScore()),
  ...
  private BigDecimal calculateFinalScore(EntranceTestResult entranceTestResult) {
    ...
    BigDecimal adjustedDocumentScore =
        documentEvaluationScore.divide(BigDecimal.valueOf(3), 3, RoundingMode.HALF_UP);

    return adjustedDocumentScore
